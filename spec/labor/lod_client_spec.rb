  ...
  describe 'post_query' do
    ...
    context 'LODQA_BSから成功レスポンスが帰ってきたとき' do
      before do
        ...
        stub_request(:post, LodqaClient::SERVER_URL).with(body: registered_query).to_return(status: 200)
      end
      it 'nilを返すこと' do
    ...
    context '異常レスポンスが帰ってきた時' do
      ...
      it 'nilを返すこと' do expect(subject.post_query(@question, @address_to_send, {})).to eq nil end
      it 'エラーメールが送信されることを確認' do allow(FailureMailer).to receive(:deliver_email) end
    end
  end
end
